...
static inline int acp_sdwdai_get_config(const struct device *dev,
...
static inline int acp_sdwdai_set_config(const struct device *dev,
					const struct dai_config *cfg,
					 const void *spec_config,
					 size_t spec_config_size)
{
	struct acp_dai_pdata *data = dev->data;
	struct acp_dai_config *acp_dai_config = data->priv_data;
...
static int acp_sdwdai_trigger(const struct device *dev,
			      enum dai_dir dir,
...
static const struct dai_properties
	*acp_sdwdai_get_properties(const struct device *dev,
...
#define AMD_DAI_ACP_SDW_INIT(inst) \
	...
	DEVICE_DT_INST_DEFINE(inst, &amd_dai_acp_sdw_init, NULL, \
			      &acp_dai_sdw_pdata_##inst, \
			      &acp_dai_sdw_config_##inst, \
			      POST_KERNEL, \
